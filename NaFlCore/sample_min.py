...
def sub_queue_min(*files):
    ...
    for file in files:
        ...
        for index, value in enumerate(file.bitmap):
            if value > 0:
                set_bits.add(index)

        cov_map[file] = copy.copy(set_bits)
        max_coverage.update(set_bits)
    ...
    while len(cov_map) > 0:
        ...
        for file, path_coverage_set in cov_map.items():
            tmp = path_coverage_set - selected_set
            if len(tmp) == 0:
                del cov_map[file]
            else:
                path_coverage_set = tmp

        min_file_set.append(rfile_selected)
